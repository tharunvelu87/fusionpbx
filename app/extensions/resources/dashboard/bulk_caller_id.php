...
//create token
	$object = new token;
	$token = $object->create($_SERVER["DOCUMENT_ROOT"].'/extensions/resources/dashboard/bulk_caller_id.php');
...
//dashboard state variables  
	...
	$dashboard_name = $_SESSION['dashboard_name'] ?? '';

//toggle and details styling
	$toggle = ($dashboard_details_state === 'disabled')
		? ''
		: " onclick=\"$('#caller_id_details').slideToggle('fast'); toggle_grid_row_end('{$dashboard_name}');\"";
	$details_style = ($dashboard_details_state === 'expanded') ? 'display:block;' : 'display:none;';

//stats for chart
	$stats['total'] = count($_SESSION['caller_ids'] ?? []);
	$stats['active'] = !empty($current_number) && $current_number !== 'None' ? 1 : 0;
